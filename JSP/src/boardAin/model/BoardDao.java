...
public class BoardDao
{
	...
	public int getGroupId() throws BoardException
	{
		...
		try{
			...
			// 2. sql
			String sql ="	SELECT seq_GROUP_ID_ARTICLE.nextval AS groupId "+
						"	FROM dual	";
			// 3. 전송객체
	...
	public int insert( BoardRec rec ) throws BoardException
	{
		...
		*/
		...
		try{
			...
			return articleId; // 입력한 글의 글번호(articleId)를 리턴
		}catch( Exception ex ){
	...
	public List<BoardRec> selectList(int firstRow, int endRow) throws BoardException
	{
		...
		try{
			...
			String sql = "select *	" + 
					"	from article	" + 
					"	where sequence_no IN(select sequence_no	" + 
					"	from(select sequence_no, rownum as rnum	" + 
					"	from(select sequence_no, group_id	" + 
					"	from(select sequence_no, group_id from article order by sequence_no DESC)	" + 
					"	order by group_id, sequence_no DESC))	" + 
					"	where rnum>=? AND rnum<=?)	";
...
//				rec.setContent(rs.getString("CONTENT"));
			...
			if( isEmpty ) return Collections.emptyList(); //Collections은 setter, getter같은 애들의 내부클래스로 얘네들을 검색해서 있나없나 확인
...
//				rec.setPassword(rs.getString("PASSWORD"));
	...
	public int getTotalCount() throws BoardException{
		...
		try {
			...
			throw new BoardException("게시판 ) db내 전체 게시물 개수 가져오기  : " + ex.toString() );
		}finally {
			if( rs   != null ) { try{ rs.close();  } catch(SQLException ex){} }
			if( ps   != null ) { try{ ps.close();  } catch(SQLException ex){} }
			if( con  != null ) { try{ con.close(); } catch(SQLException ex){} }
