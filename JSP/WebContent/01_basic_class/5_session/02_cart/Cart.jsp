...
<%
	...
	// 1. Form의 값(hidden값) 넘겨받기 ( id, name, price )j
	id = request.getParameter("id");
	name = request.getParameter("name");
	price = Integer.parseInt(request.getParameter("price"));
	// 2. 세션의 cart 속성을 얻어온다. 
	Object obj = session.getAttribute("cart"); // "cart"라는 속성을 지닌 세션을 가져오고 (object로 넘어와서)Object형으로 이를 obj로 저장함
	// 3. 만일 null이면 ArrayList 객체 새로 생성하고 그렇지 않으면 ArrayList 변수(glist)에 지정
	if(obj == null){
		glist = new ArrayList<Goods>();
	}else{
		glist = (ArrayList<Goods>)obj; // "cart"라는 속성을 지닌 세션을 가져오고(ArrayList<Goods> 형변환) 이를 glist라고 저장함
	}
